...
public class HisRoomSteps {
	...
	@Browser("hisroom")
	AspireBrowser<HisRoomPage> hisroomPage;
	
	
	@Then("[8024-0001] User compare between $valueOne and $valueTwo")  //Custom step used to compare between two double value
	...
	@When("[8024-0002] user randomly select an available $element")
	public void randomselect(String element) throws InterruptedException
	{
		try
		{   sleep("5000");
		    if(element.equals("hisroomSize"))
		    {
	...
	@Then("[8024-0003] sleep after last action for $element Milliseconds")
	@When("[8024-0003] sleep after last action for $element Milliseconds")
	...
	@When("[8024-0004] User hits $value with $data")
	...
	@Then("[8024-0005] User compare between $valueOne and $valueTwo")  //Custom step
	...
	@Then("[8024-0006] the user check the sum of $valueOne and $valueTwo with $total")  //Custom step
	...
	@When("[8024-0007] $orderTotal price should be matched with the summation of $itemTotal , $shipping and $dutiesAndTaxes")
